...
const todoTab = (() => {
  ...
  const renderNewProjectButton = () => {
    ...
    const newBtn = renderComponents.renderTag('span', 'new-project-btn', null);
    newBtn.textContent = 'New Project';
  };

  const renderNoProjectWarning = () => {
    const warning = renderComponents.renderTag('h1', 'content', null, ['box', 'col-12']);
  ...
  const renderAllProjects = (key) => {
    ...
    const project = renderComponents.renderTag('h2', `${key}`, `project-${key}`, ['minibox', 'col-12', 'col-l-10', 'project-name-box']);
    project.textContent = key.replace(/-+/g, ' ');
    const newBtn = renderComponents.renderTag('button', `${key}`, `new-task-btn-${key}`, ['col-12', 'col-l-2']);
  ...
  const renderTask = (name, description, difficulty, date, parent) => {
    // Create Tags
    renderComponents.renderTag('div', parent, `task-container-${name}`, ['box', 'task-container']);
  ...
  const swapBtns = (name, state) => {
    if (state) {
      const btn = document.getElementById(`complete-task-btn${name}`);
      btn.id = `undo-task-btn-${name}`;
      btn.innerHTML = 'Undo Task';
      document.getElementById(`task-container-${name}`).classList.add('completed-task');
    } else {
      const btn = document.getElementById(`undo-task-btn-${name}`);
      btn.id = `complete-task-btn${name}`;
      btn.innerHTML = 'Done!';
      document.getElementById(`task-container-${name}`).classList.remove('completed-task');
    }
  };
  ...
  const renderProjectsForm = () => {
    ...
    const label = renderComponents.renderTag('label', 'project-form', null, ['col-12', 'col-m-4', 'col-l-4']);
    label.textContent = 'Project Name';
    renderComponents.renderTag('input', 'project-form', 'project-name', ['col-12', 'col-m-8', 'col-l-8']);
    const submitBtn = renderComponents.renderTag('input', 'project-form', 'project-submit-btn', ['col-12']);
  ...
  const renderNewTaskForm = () => {
    ...
    // Submit btn
    const submitBtn = renderComponents.renderTag('input', 'new-task-form', 'fsubmit', ['col-11', 'col-l-4']);
    submitBtn.type = 'button';
  ...
  return {
    renderAllTasks,
    swapBtns,
    renderNoProjectWarning,
    renderProjectsForm,
    unrenderProjectsForm,
    renderAllProjects,
    renderTask,
    renderNewTaskForm,
    unrenderNewTaskForm,
